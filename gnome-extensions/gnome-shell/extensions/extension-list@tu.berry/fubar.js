...
import {has, xnor} from './util.js';

const ruin = o => o?.destroy();
const omit = (o, ...ks) => ks.forEach(k => { ruin(o[k]); delete o[k]; });
...
export class DBusProxy extends Gio.DBusProxy {
    ...
    constructor(name, object, callback, hooks, signals, xml, cancel = null, bus = Gio.DBus.session, g_flags = Gio.DBusProxyFlags.NONE) {
        ...
        if(hooks) connect(this.$hub, this, ...hooks);
...
export class Light extends Source {
    constructor(callback) {
        super(() => new DBusProxy('org.gnome.SettingsDaemon.Color', '/org/gnome/SettingsDaemon/Color', x => callback(x.NightLightActive),
            ['g-properties-changed', (x, p) => { if(p.lookup_value('NightLightActive', null)) callback(x.NightLightActive); }]));
    }
}
